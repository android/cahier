    ...
    Scaffold(
        topBar = {
            BrushDesignerTopBar(
                ...
                onLoadStockBrush = { viewModel.loadStockBrush(it) },
    ...
    Column(
        ...
        when (selectedTab) {
            ...
            BrushDesignerTab.Behaviors -> BehaviorsTabContent(
                ...
                onAddSmoothedBehavior = { source, target, damping ->
                    viewModel.addSmoothedBehavior(
                        sourceType = source,
                        targetType = target,
                        dampingSeconds = damping
                    )
                },
                onAddJitterBehavior = { target ->
                    viewModel.addJitterBehavior(target)
                }
            )
        }
    }
