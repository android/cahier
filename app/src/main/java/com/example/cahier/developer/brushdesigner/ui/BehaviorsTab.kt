...
internal fun BehaviorsTabContent(
    ...
    onAddSmoothedBehavior: (BrushBehavior.Source, BrushBehavior.Target, Float) -> Unit,
    onAddJitterBehavior: (BrushBehavior.Target) -> Unit
) {
    ...
    AdvancedDynamicsSection(
        onAddSmoothedBehavior = onAddSmoothedBehavior,
        onAddJitterBehavior = onAddJitterBehavior
    )
}
...
private fun AdvancedDynamicsSection(
    ...
    Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
        FilledTonalButton(
            ...
            onClick = {
                onAddSmoothedBehavior(
                    BrushBehavior.Source.SOURCE_NORMALIZED_PRESSURE,
                    BrushBehavior.Target.TARGET_SIZE_MULTIPLIER,
                    0.15f
                )
            }
        ) {
        ...
        FilledTonalButton(
            ...
            onClick = {
                onAddJitterBehavior(
                    BrushBehavior.Target.TARGET_SLANT_OFFSET_IN_RADIANS
                )
            }
        ) {
            Icon(
        ...
        FilledTonalButton(
            ...
            onClick = {
                onAddSmoothedBehavior(
                    BrushBehavior.Source
                        .SOURCE_SPEED_IN_MULTIPLES_OF_BRUSH_SIZE_PER_SECOND,
                    BrushBehavior.Target.TARGET_OPACITY_MULTIPLIER,
                    0.3f
                )
            }
        ) {
